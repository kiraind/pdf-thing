...
fn main() {
    ...
    let text = "Ура нашёл норм либу";
    let text2 = "охуенно бля";

    let mut font_reader = std::io::Cursor::new(include_bytes!("../Montserrat-ExtraBold.ttf").as_ref());

    let font = doc.add_external_font(&mut font_reader).unwrap();

    // `use_text` is a wrapper around making a simple string
    current_layer.use_text(text, 36, Mm(10.0), Mm(200.0), &font);
